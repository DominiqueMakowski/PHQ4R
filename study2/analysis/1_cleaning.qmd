...
df |>
  ...
  geom_text(aes(color = Experimenter, label=ifelse(Experiment_Duration < 3, as.character(Participant), '')), hjust=-1, vjust=0) +
...
`r sum(df$MoodDisorder)` participants (`r insight::format_percent(sum(df$MoodDisorder) / nrow(df))`) have a mood disorder, as indexed by the use of a mood stabilizer, antidepressant, or anxiolytic treatment. `r sum(df$DisorderHistory)` participants (`r insight::format_percent(sum(df$DisorderHistory) / nrow(df))`) reported having a history of mood disorder.
