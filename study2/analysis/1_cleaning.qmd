...
We removed `r length(outliers$attentionchecks)` participants for failing the attention checks.
...
mgcv::gam(Failed ~ s(STAI5_SD), 
          data=mutate(df, Failed = ifelse(AttentionCheck == "Failed", 1, 0)), 
          family="binomial") |>
  parameters::parameters()

df |> 
  select(Participant, STAI5_SD, AttentionCheck) |> 
  mutate(Participant = fct_reorder(Participant, STAI5_SD)) |>
  ggplot(aes(x=Participant, y=STAI5_SD, fill=AttentionCheck)) +
  geom_bar(stat="identity") +
  theme_minimal() +
  theme(axis.text.x = element_blank())
```
...
```{r}
dfoutlier <- df |> 
  select(contains("IAS_"), -IAS_Duration, -IAS_Order, -IAS_Total) |>
  ...
  mutate(Participant = fct_reorder(df$Participant, Distance_OPTICS),
         ...
         Outlier_Distance = ifelse(abs(standardize(Distance_OPTICS)) > qnorm(0.995), 1, 0),
...
```{r}
df <- filter(df, !Participant %in% c(outliers$attentionchecks, outliers$distance))
...
df$Depression <- rowSums(select(df, DisorderTreatment_MoodStabilizer, DisorderTreatment_Antidepressant))
df$Depression <- ifelse(df$Depression > 0, TRUE, FALSE)

df$DisorderHistory <- ifelse(df$DisorderHistory == "Yes", TRUE, FALSE)
```

`r sum(df$MoodDisorder)` participants (`r insight::format_percent(sum(df$MoodDisorder) / nrow(df))`) have a mood disorder, as indexed by the use of a mood stabilizer, antidepressant, or anxiolytic treatment. `r sum(df$Depression)` participants (`r insight::format_percent(sum(df$Depression) / nrow(df))`) have a mood disorder, as indexed by the use of a mood stabilizer or antidepressant. `r sum(df$DisorderHistory, na.rm=TRUE)` participants (`r insight::format_percent(sum(df$DisorderHistory, na.rm=TRUE) / nrow(df))`) reported having a history of mood disorder.
