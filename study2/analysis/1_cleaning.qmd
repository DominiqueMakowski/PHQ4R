...
# TODO: compute modified score
# Maybe use the score_PHQ4 function from the README to demo it here
...
df <- df |>
  mutate(
    ...
    Expe_PHQ4_Anxiety = ifelse(
      PHQ4_Condition == "PHQ4 - Revised", datawizard::rescale(Expe_PHQ4_Anxiety_1 + Expe_PHQ4_Anxiety_2, range = c(0, 2), to = c(0, 6)), PHQ4_Anxiety_1 + PHQ4_Anxiety_2
    ),
    Expe_PHQ4_Depression = ifelse(
      PHQ4_Condition == "PHQ4 - Revised", datawizard::rescale(Expe_PHQ4_Depression_3 + Expe_PHQ4_Depression_4, range = c(0, 2), to = c(0, 6)), PHQ4_Depression_3 + PHQ4_Depression_4
    ),
    Expe_PHQ4_Total = Expe_PHQ4_Anxiety + Expe_PHQ4_Depression
  )

# df |>
#   select(starts_with("Expe_PHQ4_"), PHQ4_Condition) |>
...
df |>
  ...
  geom_point(aes(color = Experimenter, shape = Category)) +
  geom_text(aes(color = Experimenter, label=ifelse(Experiment_Duration < 4.5, as.character(Participant), '')), hjust=-1, vjust=0) +
  scale_shape_manual(values = c("extra" = 3, ok = 19)) +
  guides(shape = "none") +
  ggside::geom_xsidedensity(aes(fill=Experimenter), color=NA, alpha=0.5) +
...
# df |> 
#   ggplot(aes(x=BDI2_Duration, y=BDI2_SD)) +
#   geom_point() 
...
```{r}
df |> 
  select(starts_with("IAS_"), -IAS_Duration, -IAS_Order) |>
  pivot_longer(starts_with("IAS_"), names_to = "item", values_to = "score") |> 
  ggplot(aes(x = score)) +
  geom_density() +
  facet_wrap(~item)
```
...
add_bf <- function(r) {
  x <- as.data.frame(r)
  f <- as.formula(paste(str_remove(x$Parameter, fixed("df$")), "~ PHQ4_Condition"))
...
rbind(
  add_bf(report::report(t.test(df$Age ~ df$PHQ4_Condition))),
  add_bf(report::report(t.test(df$PHQ4_Total ~ df$PHQ4_Condition))),
  add_bf(report::report(t.test(df$STAI5_General ~ df$PHQ4_Condition))),
  add_bf(report::report(t.test(df$BDI2_Total ~ df$PHQ4_Condition)))
) |> 
