...
The reliability of the anxiety ($Cronbach's~\alpha = 0.903$; RMSEA = 0.031) and depression ($Cronbach's~\alpha = 0.841$; RMSEA = 0.044) subscales are excellent (>0.8). The proportion of response types stratified by item (see **Figure 1A**) shows that the new "Once or twice" option we added was the most prevalent response for all items (on average selected in **29.12\%** of cases).
...
The second scoring method we propose takes into account the non-linear spacing between responses, as indicated by the peak of their measurement information. To develop this measurement, we found for each dimension the lower and higher values of the latent trait $\theta$ for which the probability of identification is 95\%. **In other words, the probability that an individual responds in a particular category given a trait level is 95%. (not sure if this is what this means?)** We considered these points at 0 and 1, and normalized the location of the peaks of each response in between.
